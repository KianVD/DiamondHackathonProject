...
def generate_natural_language_explanation(user_features, card_number):
    # Extract data for the specified card_number.
    user_data = user_features[user_features['card_number'] == card_number].iloc[0]
    input_text = (
...
def generate_concise_report(user_features, card_number, pdf_filename='reports/concise_credit_risk_report.pdf'):
    # Extract data for the specified card_number.
    user_data = user_features[user_features['card_number'] == card_number].iloc[0]
    ...
    prompt = (
        ...
        f"Total Transaction Amount: {user_data['total_amount']}, "
        f"Transaction Count: {user_data['transaction_count']}, "
        f"Average Transaction Amount: {user_data['avg_amount']}, "
        f"Predicted Credit Risk: {user_data['credit_risk']}. "
    ...
    # Display key metrics.
    metrics = (
        f"<b>Total Transaction Amount:</b> {user_data['total_amount']}<br/>"
        f"<b>Transaction Count:</b> {user_data['transaction_count']}<br/>"
        f"<b>Average Transaction Amount:</b> {user_data['avg_amount']}<br/>"
        f"<b>Predicted Credit Risk:</b> {user_data['credit_risk']}"
    )
